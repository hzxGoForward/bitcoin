...
void mempoolStatics(const int blkHeight)
{
    ...
    std::string msg = now + " " + std::to_string(poolsize) + " " + std::to_string(txcnt) + " \n";
...
int getPredTxSet(const std::shared_ptr<const CBlock>& pblock, const int h, std::set<uint256>& predTxidSet)
{
    ...
    for (size_t i = 1; i < pblock->vtx.size(); ++i) {
        ...
        if (umap_setPredictTxid[h].count(txid)) {
            mi = std::max(mi, umap_setPredictTxid[h][txid]);
        }
    }
    if (mi == -1)
...
/// <summary>
/// 遍历新区块中的交易，如果本地不存在该交易，则放入交易池.
/// 将本地不存在的交易放入mapMissTxid中，另外将新区块中除了coinbase交易以外的所有交易放入 mapBlkTxidIndex
void acceptMissTxToMemPool(const std::shared_ptr<const CBlock>& pblock, std::ostringstream& os, std::map<uint256, int>& mapPredTxIndex, std::set<uint256>& predTxidSet, std::map<CTransactionRef, size_t>& mapMissTxid, std::map<uint256, int>& mapBlkTxidIndex)
{
    os << "entertime txid fee txSz txWeight position\n";
    // 写入coinbase交易信息
    const auto now = FormatISO8601DateTime(GetTime());
    const uint256& cbTxid = pblock->vtx[0]->GetHash();
    const CAmount cbFee = 0;
    const size_t cbTxSz = pblock->vtx[0]->GetTotalSize();
    os << format("%s %s %lld %d %d %s\n", now.data(), cbTxid.ToString().data(), cbFee, cbTxSz, 0, std::string("None").data());
    mapMissTxid[pblock->vtx[0]] = 0;
    const int predSz = predTxidSet.size();                              // 参与预测序列的交易数量
    // 遍历pblock中每一笔交易，并将信息存入os中
    for (size_t i = 1; i < pblock->vtx.size(); ++i) {
        std::string entertime = now, pos = "Seq";
        const uint256& txid = pblock->vtx[i]->GetHash();
        CAmount fee = 0;
        size_t txSz = 0, txWeight = 0;
        // 如果不存在于预测序列中，则放入miss交易序列
        if (mapPredTxIndex.count(txid) == 0) {
            // 标记为None，并且插入预测序列，将交易哈希值对应索引放入mapMissTxid中
            pos = "None";
            predTxidSet.insert(txid);
            mapMissTxid[pblock->vtx[i]] = i;
            ...
            pos += "_"+ std::to_string(mapPredTxIndex[txid]);
        }

         // 交易可能存在于predSet中，但是不存在于交易池中,因此最后还需要再次确认
        // 如果交易不存在于本地交易池，则放入交易池中
        if (mempool.exists(txid) == false) {
            CValidationState state;
            bool fMissingInputs = false;
            std::list<CTransactionRef> lRemovedTxn;
            bool accept = AcceptToMemoryPool(mempool, state, pblock->vtx[i], &fMissingInputs, &lRemovedTxn, false /* bypass_limits */, 0 /* nAbsurdFee */);
            // mempool.check(&::ChainstateActive().CoinsTip());
            if (accept == false) {
                pos += format("_REJECT_%s", state.GetDebugMessage().data());
            } else {
                pos += "_ACCEPT";
            }
        }
...
/// <summary>
/// 根据predTxidSet中的交易，以及新区快中的所有交易，生成预测交易序列,并且将除了coinbase交易以外的所有信息写入tmps中
/// </summary>
/// <param name="tmps">存放预测序列信息的stream</param>
/// <param name="predTxidSet">预测交易集合</param>
/// <param name="mapBlkTxidIndex">新区块的交易集合</param>
/// <param name="mapPredTxidIndex">预测交易哈希值到其索引的map映射</param>
/// <returns></returns>
std::unique_ptr<CBlockTemplate> createPredBlock(std::ostringstream& tmps, std::set<uint256>& predTxidSet, std::map<uint256, int>& mapBlkTxidIndex, std::map<uint256, int> mapPredTxidIndex)
{
    ...
    for (int i = 1; i < blk->block.vtx.size(); ++i) {
        ...
        auto it = mempool.mapTx.find(txid);
        const std::string entertime = FormatISO8601DateTime(it->GetTime());
        tmps << format("%s %s %lld %d %d %d %d %f\n", entertime.data(), txid.ToString().data(), it->GetFee(), it->GetTxSize(), it->GetTxWeight(), i - 1, blk->vTxGroup[i], blk->vTxFeeRate[i]);
        // 基于预测交易序列的哈希值
...
void compareBlock(const std::shared_ptr<const CBlock>& pblock, const int h)
{
    ...
    if (pblock->vtx.size() == 1) {
        ...
        size_t sz = GetSerializeSize(pblock, PROTOCOL_VERSION);
        const std::string strTxid = pblock->GetHash().ToString();
        ss2 << format("block_sz(bytes): %d recv_time: %s\nblock_hash: %s tx_cnt: %d\n", sz, now.data(), strTxid.data(), pblock->vtx.size());
        writeFile(std::to_string(h) + "_new_block.log", ss2.str());
        return;
    }
    
    // 1. 求出预测序列中的索引mi，将[0,mi]范围内的预测交易序列放入predTxidSet中
    std::set<uint256> predTxidSet;
    int lastIndex = getPredTxSet(pblock, h, predTxidSet);
    
    // 2. 求出预测序列中缺失的交易,将其放入mapMissTxid中，并放入交易池中
    std::map<CTransactionRef, size_t> mapMissTxid; // mapMissTxid 存储缺失交易及其索引
    std::map<uint256, int> mapBlkTxidIndex;        // mapBlkTxidIndex 记录每一笔新区块中交易的哈希值以及对应的索引
    std::ostringstream blkOs;
    acceptMissTxToMemPool(pblock, blkOs, umap_setPredictTxid[h], predTxidSet, mapMissTxid, mapBlkTxidIndex);
    blkOs << "predTxidSet_Range: " << lastIndex << std::endl; 
    // 3. 利用predTxidSet生成预测序列
    std::ostringstream predOs;
    std::map<uint256, int> mapPredTxidIndex;
    auto predBlk = createPredBlock(predOs, predTxidSet, mapBlkTxidIndex, mapPredTxidIndex);

    // 使用多线程写数据，然后保存预测区块、比较结果、预测的交易序列
    std::thread th1(writeFile, std::to_string(h) + "_new_block.log", blkOs.str());
    std::thread th2(writeFile, std::to_string(h) + "_pred_block.log", predOs.str());

    // 使用Dino协议计算代价
    // DinoBlock dino_block(*pblock, predBlk->block, mapMissTxid);
    // dino_block.dinoProtocol();
